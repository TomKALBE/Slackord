...
<template>
    <div class="w-full">
        <div id="chat" class="flex-col-reverse chat-max-height rounded-md w-12/12 mx-3 overflow-y-scroll scrollbar">
            ...
            <div v-if="conversationMessages[getReceiverIdIndex(props.user.id)]"
                v-for="message in conversationMessages[getReceiverIdIndex(props.user.id)].messages" :key="message.id"
