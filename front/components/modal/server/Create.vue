            ...
            <div class="relative rounded-lg shadow bg-slate-800 border-2 border-slate-600">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div v-else-if="joinServer">
                        <label for="first_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nom du
                            serveur</label>
                        <input v-model="serverName" type="text" id="first_name"
                            class="bg-gray-50 border border-gray-300 dark:bg-gray-700 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                ...
                <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b border-gray-600">
                    <button type="button"
                        ...
                        v-if="!joinServer" @click="createServer ? createNewServer() : createServer = !createServer">
                        Créer un serveur
                    </button>
                    <button @click="joinServer ? joinNewServer() : joinServer = !joinServer" v-if="!createServer"
                        type="button"
                        class="text-white bg-rose-600 active:bg-rose-700 duration-300 hover:scale-105 tran focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                        Rejoindre un serveur
                    </button>
                    <button v-if="createServer || joinServer" @click="createServer = false; joinServer = false"
                        type="button"
